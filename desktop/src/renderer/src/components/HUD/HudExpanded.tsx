      ...
      return (
        <>
          ...
          <div className="mt-4 flex gap-5">
            {(['vscode', 'spotify'] as const).map(id => {
              const status = connectorStatuses.find(s => s.id === id)
              ...
              return (
                ...
                >
                  ...
                  >
                    {id === 'vscode' ? 'Vs Code' : 'Spotify'}
                  </span>
                  <button
                    onMouseDown={e => e.preventDefault()}
                    disabled={connected || (id === 'vscode' && (installingVSCode || vscodePendingReload))}
                    onClick={() => {
                      if (connected || id === 'spotify') return
                      setVscodeInstallMsg(null)
                      setInstallingVSCode(true)
                      window.bridgeAPI.installVSCodeConnector().then(res => {
                        setInstallingVSCode(false)
                        if (res.ok) {
                          setVscodePendingReload(true)
                          setVscodeInstallMsg(res.message)
                        } else {
                          setVscodeInstallMsg(res.message)
                          setTimeout(() => setVscodeInstallMsg(null), 6000)
                        }
                      })
                    }}
                    ...
                    style={{
                      ...
                      borderRadius: 6,
                      border: 'none',
                      background: connected ? 'rgba(255,255,255,0.18)' : 'rgba(255,255,255,0.92)',
                      color: connected ? 'rgba(255,255,255,0.7)' : 'rgba(0,0,0,0.85)',
                      cursor: connected ? 'default' : 'pointer',
                      letterSpacing: '-0.01em',
                      fontWeight: 500
                    }}
                  >
                    {id === 'vscode' && installingVSCode
                      ? 'Instalando…'
                      : id === 'vscode' && vscodePendingReload
                        ? 'Aguardando…'
