    ...
    <div className="flex flex-col gap-0 w-full">
          ...
          <div key={step.id} className="flex items-stretch gap-2">
            ...
            <div className="flex flex-col items-center" style={{ minWidth: 18 }}>
              <span
                ...
                style={{
                  color: isRun ? 'var(--ares-accent)' : isDone ? 'var(--ares-text-tertiary)' : 'var(--ares-text-muted)',
                  animation: isRun ? 'ares-pulse 1.2s ease-in-out infinite' : undefined
                }}
              >
                {STEP_ICONS[step.id] ?? '◆'}
              </span>
              ...
              {(!isLast || streamingContent) && (
                <div
                  ...
                  style={{ background: isDone ? 'var(--ares-border-soft)' : 'color-mix(in srgb, var(--ares-border-soft) 50%, transparent)', minHeight: 12 }}
                />
              )}
            </div>
      ...
      {streamingContent && (
        <div className="flex items-start gap-2">
          <div className="flex flex-col items-center" style={{ minWidth: 18 }}>
            <div className="w-[6px] h-[6px] rounded-full mt-[5px]" style={{ background: 'var(--ares-accent)' }} />
          </div>
          <p
            className="text-[13px] leading-relaxed flex-1 pb-1"
            style={{ color: 'var(--ares-text-primary)', whiteSpace: 'pre-wrap', wordBreak: 'break-word', letterSpacing: 'var(--hud-body-tracking, -0.014em)' }}
          >
            {streamingContent}
            <span
              className="inline-block w-[2px] h-[13px] ml-[2px] align-text-bottom rounded-sm"
              style={{ background: 'var(--ares-accent)', animation: 'ares-blink 1s step-end infinite' }}
            />
          </p>
        </div>
      )}
